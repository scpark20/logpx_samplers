...
if config.backbone == 'EDM':
    model = EDM(trainable=True)  # 내부 구현에 맞춰 유지
    model.set_freeze()
device = model.device
print(model)
...
def get_classifier_loss(raw_output):
    loss_list = []
    for classifier in classifiers:
        loss = classifier.entropy(raw_output)['loss']
        loss_list.append(loss)
    return torch.mean(torch.stack(loss_list))

@torch.no_grad()
def get_valid_loss(valid_noises, solver):
    solver.eval()
    losses = []
    start = 0
    while True:
...
def do_train_loop(device, writer, solver, optimizer, global_step):
    ...
    for _, batch in enumerate(pbar):
        ...
        if config.main_loss == 'classifier':
            seeds = np.random.randint(np.iinfo(np.int32).max, size=(config.batch_size,))
...
def main():
    ...
    global_step = 0
    seeds = np.random.randint(np.iinfo(np.int32).max, size=(config.n_valid,))
    valid_noises = model.get_noise(seeds=seeds)
    while True:
        loss = get_valid_loss(valid_noises, solver)

...
def get_valid_loss(valid_loader, device, solver):
    ...
    for batch in valid_loader:
        ...
        with torch.no_grad():
            ...
            student_out = model.decode_vae(latent_pred, raw_output=True)['raw_output']
            teacher_out = model.decode_vae(targets, raw_output=True)['raw_output']
            inception_loss = F.mse_loss(inception.encode(student_out), inception.encode(teacher_out)) 
            losses.append(inception_loss.item())
...
def do_train_loop(device, train_loader, solver, optimizer, global_step):
    ...
    for batch in pbar:
        ...
        if config.main_loss == 'epd_loss':
            target_traj = interp_traj(teacher_traj, teacher_timesteps, outputs['timesteps'])
            mse_loss = F.mse_loss(target_traj[:, :-1], outputs['traj'][:, :-1])

            student_out = model.decode_vae(outputs['samples'], raw_output=True)['raw_output']
            teacher_out = model.decode_vae(targets, raw_output=True)['raw_output']
            inception_loss = F.mse_loss(inception.encode(student_out), inception.encode(teacher_out)) 

            print('mse_loss :', mse_loss, 'inception_loss :', 10*inception_loss)
            loss = mse_loss + 10*inception_loss

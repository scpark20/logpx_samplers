...
PerceptualSimilarity = os.path.join("submodules", "PerceptualSimilarity")
...
lpips_loss_fn = lpips.LPIPS(net='squeeze') # closer to "traditional" perceptual loss, when used for optimization
...
def LPIPS_LOSS(latents1, latents2, decoder):
    images1 = decoder(latents1)
    images2 = decoder(latents2)
    return lpips_loss_fn(images1, images2)
...
def get_valid_loss(valid_loader, device, solver):
    ...
    for batch in valid_loader:
        ...
        with torch.no_grad():
            ...
            loss = LPIPS_LOSS(latent_pred, targets, lambda x: model.decode_vae(x, raw_output=True)['raw_output'])
            loss = loss.mean()

...
def get_args():
    ...
    p.add_argument('--tau_temp',    type=float, default=1.0)
...
from solvers.taylor.transform.loglinear_transform8 import LogLinearTransform
from solvers.taylor.extractor.table_extractor import Extractor

noise_schedule = model.get_noise_schedule()
extractor = Extractor(steps=config.n_steps)
transform = LogLinearTransform(gamma_push=True, gamma_init=config.gamma_init, tau_init=config.tau_init, tau_temp=config.tau_temp, eps=1e-2)
solver = GDual_Solver(
    noise_schedule,

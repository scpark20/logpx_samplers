...
config.train_pt_dir  = '/dataset/gmdit/train1.4_1k_traj'
...
class SyntheticDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        traj = torch.zeros((201, self.C, self.H, self.W))
        timesteps = torch.linspace(1, 1e-3, 201)
        cond = torch.randint(1000, size=(1,))
        return {'noise': noise, 'sample': target, 'cond': cond, 'traj': traj, 'timesteps': timesteps}

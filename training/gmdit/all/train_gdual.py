...
def do_train_loop(device, writer, solver, optimizer, global_step):
    ...
    for _, batch in enumerate(pbar):
        ...
        model_fn = model.get_model_fn(noise_schedule, pos_conds=conds, guidance_scale=config.CFG)
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            outputs = solver.sample(noises, model_fn, output_x0=True)
            if 'classifier' in config.main_loss:
                # (b, steps+1, c, h, w)
                x0_list = outputs['x0_list']
                b1, b2, c, h, w = x0_list.shape
                decoder_inputs = x0_list.reshape(b1*b2, c, h, w)
                outputs = model.decode_vae(decoder_inputs, raw_output=True)
                targets = conds.repeat_interleave(b2)
                loss = get_classifier_loss(outputs['raw_output'], targets=targets)

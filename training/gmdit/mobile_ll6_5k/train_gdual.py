...
def get_args():
    ...
    p.add_argument('--tau_x_init',    type=float, default=0.0)
    p.add_argument('--tau_e_init',    type=float, default=0.0)
...
config.tau_x_init      = args.tau_x_init
config.tau_e_init      = args.tau_e_init
...
transform = LogLinearTransform(gamma_push=True,
                            gamma_init=config.gamma_init,
                            tau_x_init=config.tau_x_init,
                            tau_e_init=config.tau_e_init,
                            eps=1e-2)
solver = GDual_Solver(
    noise_schedule,

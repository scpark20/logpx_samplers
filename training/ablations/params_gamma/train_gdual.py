...
def get_args():
    ...
    p.add_argument('--gamma_p1', action='store_true',)
    p.add_argument('--gamma_0', action='store_true',)
    p.add_argument('--gamma_n1', action='store_true',)
    return p.parse_args()

args = get_args()

# 하나만 선택(또는 0개) 강제
only_one = int(args.gamma_p1) + int(args.gamma_0) + int(args.gamma_n1)
if only_one > 1:
    raise ValueError("Choose at most one of: --gamma_p1, --gamma_0, --gamma_n1")
...
from solvers.taylor.transform.ablation_gamma_logaffine_transform import LogAffineTransform
...
transform = LogAffineTransform(
    gamma_push=True, gamma_max=2, tau_offset=1, kappa_max=2, eps=1e-2, gamma=gamma)
solver = GDual_Solver(
    noise_schedule,

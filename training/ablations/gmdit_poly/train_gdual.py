...
from solvers.taylor.extractor.poly_time_extractor import Extractor

noise_schedule = model.get_noise_schedule()
extractor = Extractor(poly_order=config.poly_order)
...
def get_valid_loss(valid_noises, valid_conds, device, solver):
    ...
    while True:
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            latent_pred = solver.sample(noises, model_fn, steps=config.steps_list[-1])['samples']
            if 'classifier' in config.losses:
...
def do_train_loop(device, writer, solver, optimizer, global_step):
    ...
    for _, batch in enumerate(pbar):
        ...
        model_fn = model.get_model_fn(noise_schedule, pos_conds=conds, guidance_scale=config.CFG)
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            latent_pred = solver.sample(noises, model_fn, steps=np.random.choice(config.steps_list))['samples']

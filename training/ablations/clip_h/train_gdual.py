...
if 'clip' in config.losses:
    clip = OpenCLIPEmbedder(model_name='ViT-H-14-378-quickgelu',
                            pretrained="dfn5b")
print('done')
...
# pip install torchvision  (설치 안돼있다면)
from torchvision.models import ResNet50_Weights  # 또는 ViT_B_16_Weights 등

classes = ResNet50_Weights.DEFAULT.meta["categories"]  # 길이 1000짜리 list[str]
...
def get_prompt(id):
    return f'A photo of a {id2label[id]}.'
    ...
    while True:
        ...
        with torch.no_grad(), torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            if 'clip' in config.losses:
                ...
                loss = clip.get_clip_loss(outputs['raw_output'], [get_prompt(i.item()) for i in conds])

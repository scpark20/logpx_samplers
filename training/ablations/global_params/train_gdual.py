...
def get_args():
    ...
    p.add_argument('--global_gamma', action='store_true',)
    p.add_argument('--global_tau', action='store_true',)
    p.add_argument('--global_kappa', action='store_true',)
    return p.parse_args()

args = get_args()

# 하나만 선택(또는 0개) 강제
only_one = int(args.global_gamma) + int(args.global_tau) + int(args.global_kappa)
if only_one > 1:
    raise ValueError(
        "Choose at most one of: "
        "--global_gamma, --global_tau, --global_kappa"
    )
...
from solvers.taylor.extractor.global_table_extractor import Extractor

noise_schedule = model.get_noise_schedule()
extractor = Extractor(steps=config.n_steps,
                      global_gamma=config.global_gamma,
                      global_tau=config.global_tau,
                      global_kappa=config.global_kappa)

...
class LogLinearTransform(Transform):
    ...
    def unpack(self, params):
        ...
        tau_x = torch.clamp(tau_x + self.tau_init, min=0.0-self.eps, max=1.0+self.eps)
        tau_e = torch.clamp(tau_e + self.tau_init, min=0.0-self.eps, max=1.0+self.eps)

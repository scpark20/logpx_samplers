...
class GDual_Solver(Solver):
    ...
    def get_next_sample(
        ...
        if order == 1:
            ...
            #print('1 :', xc.shape, coeff_X.shape)    
            X = xc * coeff_X[:, :, None, None]; E = ec * coeff_E[:, :, None, None]
            
        elif order == 2:
            X = xc * deltay_x[:, :, None, None]; E = ec * deltay_e[:, :, None, None]
            coeff_X = deltay_x; coeff_E = deltay_e
            ...
            if corrector:
                X = X + 0.5 * (xn - xc) * coeff_X[:, :, None, None]
                E = E + 0.5 * (en - ec) * coeff_E[:, :, None, None]
            else:
                ...
                X = X + 0.5 * (xc - xp)/r_u * coeff_X[:, :, None, None]
                E = E + 0.5 * (ec - ep)/r_v * coeff_E[:, :, None, None]
                
        sample_coeff, grad_coeff = self.transform.get_sample_grad_coeff(
            i, log_alpha, log_sigma, log_alpha_ratio, log_sigma_ratio, p)
        out = sample_coeff[:, :, None, None] * sample + grad_coeff[:, :, None, None] * (X + E)
        return out

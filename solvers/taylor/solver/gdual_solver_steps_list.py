...
from ..extractor.poly_time_extractor import Extractor as PolyExtractor
from ..extractor.quad_time_extractor import Extractor as QuadExtractor
from ..extractor.legendre_time_extractor import Extractor as LegendreExtractor
...
class GDual_Solver(Solver):
    def __init__(
        ...
        **kwargs
        ...
        if self.param_extractor is None:
            if 'Quad' in solver:
                self.param_extractor = QuadExtractor()
            elif 'Legendre' in solver:
                self.param_extractor = LegendreExtractor()
            elif 'Poly' in solver:
                self.param_extractor = PolyExtractor(poly_order=poly_order)
        ...
        self.log_deltas_map = nn.ParameterDict()
        for steps in steps_list:
            timesteps = self.get_time_steps(skip_type=skip_type, t_T=t_T, t_0=t_0, N=steps, device='cpu', shift=flow_shift)
            log_deltas = nn.Parameter(torch.log(timesteps[:-1] - timesteps[1:]))
            self.log_deltas_map[str(steps)] = log_deltas
    ...
    def learned_timesteps(self, steps, device=None, dtype=None):
        log_deltas = self.log_deltas_map[str(steps)]
        if device is None: device = log_deltas.device
        if dtype  is None: dtype  = log_deltas.dtype
        ...
        w = F.softmax(log_deltas, dim=0)          # (S,)
    ...
    # ---------- sampling ----------
    def sample(self, x, model_fn, steps=None, **kwargs):
        if steps is None:
        ...
        timesteps = self.learned_timesteps(steps, device=device, dtype=dtype)
        ...
        with context:
            ...
            #for i in tqdm(range(steps), disable=not use_tqdm):
            for i in range(steps):
                pred_order = min(i + 1, steps - i, self.pred_order) if self.lower_order_final else min(i + 1, self.pred_order)

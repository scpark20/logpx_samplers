...
class BNS_Dual_Solver(Solver):
    def __init__(
        ...
        self.b_data = nn.Parameter(torch.ones(steps, steps))
        self.b_noise = nn.Parameter(torch.ones(steps, steps))
    ...
    def sample(self, x, model_fn, **kwargs):
        ...
        for i in tqdm(range(self.steps), disable=os.getenv("TQDM", "False")):
            data, noise = self.checkpoint_model_fn(x, timesteps[i])
            datas.append(data)
            noises.append(noise)

            x = x0 * self.a[i]
            for j in range(0, i+1):
                x = x + delta_alphas[j]*datas[j]*self.b_data[i, j] + \
                        delta_sigmas[j]*noises[j]*self.b_noise[i, j]

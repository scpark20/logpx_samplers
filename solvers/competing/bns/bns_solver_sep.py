...
class BNS_Solver(Solver):
    def __init__(
        ...
        **kwargs
        ...
        self.bx = nn.Parameter(torch.ones(steps, steps))
        self.be = nn.Parameter(torch.ones(steps, steps))
    ...
    def sample(self, x, model_fn, **kwargs):
        ...
        for i in tqdm(range(self.steps), disable=os.getenv("TQDM", "False")):
            ...
            xs.append(delta_alphas[i]*xc)
            es.append(delta_sigmas[i]*ec)

            x = x0 * self.a[i]
            for j in range(0, i+1):
                x = x + (xs[j] * self.bx[i, j] + es[j] * self.be[i, j]) * dt[j]

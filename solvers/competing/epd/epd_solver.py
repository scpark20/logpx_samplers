    ...
    def sample(self, x, model_fn, output_traj=False, **kwargs):
        ...
        for i in range(self.steps):
            ...
            for k in range(self.K):
                ...
                eval_rho = s * tau
                eval_t = self.noise_schedule.inverse_rho(eval_rho)
                noise_k = self.checkpoint_model_fn(x_k, eval_t) if self.checkpoint else self.model_fn(x_k, eval_t)
                
                # Cum
                noise_cum = noise_cum + l[k]*noise_k

...
class GDual_Hybrid2_Solver(Solver):
    def __init__(
        self,
        model_fn,
        noise_schedule,
    ...
    def u(self, alpha, sigma, gamma):
        if gamma >= 0:
            return alpha * sigma**(-gamma), False
        else:
            return torch.log(alpha**(1 + gamma)), True

    def v(self, alpha, sigma, gamma):
        if gamma >= 0:
            return torch.log(sigma**(1 - gamma)), True
        else:
            return sigma * alpha**gamma, False

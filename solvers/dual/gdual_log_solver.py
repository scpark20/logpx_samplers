...
class GDual_Log_Solver(Solver):
    ...
    def u(self, alpha, sigma, gamma):
        return torch.log(alpha) - gamma*torch.log(sigma) if gamma >= 0 else (1+gamma)*torch.log(alpha)

    def v(self, alpha, sigma, gamma):
        return (1-gamma)*torch.log(sigma) if gamma >= 0 else gamma*torch.log(alpha) + torch.log(sigma)
    ...
    def get_next_sample(self, sample, xc, xp, ec, ep, i, alphas, sigmas, gamma, tau_x, tau_e, order):
        ...
        X = xc * torch.expm1(-delta_u)
        E = ec * torch.expm1(-delta_v)
        
        if r_u is not None and order == 2:
            if not torch.isnan(r_u):
                X = X - (xc - xp) / (2 * r_u) * self.B(delta_u, tau_x)
            if not torch.isnan(r_v):
                E = E - (ec - ep) / (2 * r_v) * self.B(delta_v, tau_e)
            
        sample_coeff = (sigmas[i + 1] / sigmas[i]) ** gamma if gamma >= 0 else (alphas[i + 1] / alphas[i]) ** (-gamma)
        X_coeff, E_coeff = -alphas[i + 1], -sigmas[i + 1]

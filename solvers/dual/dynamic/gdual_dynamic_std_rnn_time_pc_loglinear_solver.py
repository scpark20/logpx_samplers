...
class GDualSolver(Solver):
    ...
    def __init__(
        ...
        self.feat = nn.Linear(latent_shape[0]*2, hidden_dim)
    ...
    def get_params(self, xc, hidden, t_pair):
        ...
        """
        means = torch.mean(xc, dim=[2, 3])
        stds = torch.std(xc, dim=[2, 3])
        feat = self.feat(torch.cat([means, stds], dim=1))
        hidden = self.act(feat + self.time_info(t_pair) + self.hidden_info(hidden))
        out = self.out(hidden)              # [B,10]

...
class GDual_Dynamic_Double_RNN_Time_PC_LogLinear_Solver(Solver):
    """
    ...
    def __init__(
        ...
        # feature -> hidden
        self.feat1 = nn.Sequential(OrderedDict([
            ("gap",  nn.Conv2d(latent_shape[0], 1, kernel_size=(1, 1))),  # [B,C,H,W] -> [B,C,1,1]
            ("flat", nn.Flatten(1)),            # [B,C,1,1] -> [B,C]
            ("proj", nn.Linear(latent_shape[1]*latent_shape[2], hidden_dim)),
        ]))
        self.feat2 = nn.Sequential(OrderedDict([
            ("gap",  nn.Conv2d(latent_shape[0], 1, kernel_size=(1, 1))),  # [B,C,H,W] -> [B,C,1,1]
            ("flat", nn.Flatten(1)),            # [B,C,1,1] -> [B,C]
            ("proj", nn.Linear(latent_shape[1]*latent_shape[2], hidden_dim)),
        ]))
    ...
    # ---------- params ----------
    def get_params(self, xc, ec, hidden, t_pair):
        """
        xc: [B,C,H,W]
        ...
        """
        B = xc.size(0)
        h1 = self.feat1(xc)  # [B,hidden]
        h2 = self.feat2(ec)  # [B,hidden]
        if t_pair.dim() == 1:  # [2] -> [B,2]
            t_pair = t_pair.view(1, -1).expand(B, -1)
        t_pair = t_pair.to(h1.device, dtype=h1.dtype)
        h = self.act(h1 + h2 + self.time_info(t_pair) + self.hidden_info(hidden))
        out = self.out(h)              # [B,10]
    ...
    def sample(self, x, model_fn, **kwargs):
        ...
        hidden = torch.zeros(len(xc), self.hidden_dim, device=device, dtype=dtype)
        params, hidden = self.get_params(xc, ec, hidden, timesteps[0:2])
        ...
        for i in tqdm(range(self.steps), disable=not use_tqdm):
            ...
            if i < self.steps - 1:
                ...
                params, hidden = self.get_params(xn, en, hidden, timesteps[i+1:i+3])
            else:
                break  # 마지막 스텝

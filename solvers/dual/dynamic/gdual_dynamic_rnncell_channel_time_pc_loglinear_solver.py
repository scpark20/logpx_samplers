...
class GDual_Dynamic_RNNCell_Channel_Time_PC_LogLinear_Solver(Solver):
    """
    GDual PC with Log-Linear L(y, τ)=τ·log y + (1-τ)·y
    - 전역 hidden 하나로부터 채널별 파라미터 직접 예측: params [B, 2, 5, C]
    - RNNCell로 hidden 업데이트(입력: GAP+FC(C→H) + time_embed)
    - u/v 로그-도메인, pow 제거
    """
    def __init__(
        self,
        ...
        # latent shape
        assert isinstance(latent_shape, (tuple, list)) and len(latent_shape) == 3
        ...
        # hidden -> channelwise params: 2*5*C = 10*C (고정 크기, lazy 아님)
        self.out = nn.Linear(hidden_dim, 10 * self.C)
        with torch.no_grad():
    ...
    def get_params(self, xc, hidden, t_pair):
        ...
        hidden: [B,H]
        t_pair: [2] or [B,2]
        return: params [B, 2, 5, C], new_hidden [B,H]
        """
        B, C = xc.shape[:2]
        ...
        out = self.out(new_hidden)            # [B, 10*C]
        out = out.view(B, 2, 5, C)            # [B, 2, 5, C]
    ...
    def get_next_sample(
        ...
        # step logs (scalars per step; broadcastable)
        la_c, la_n = log_alpha[i],   log_alpha[i + 1]
        ls_c, ls_n = log_sigma[i],   log_sigma[i + 1]

        # logy current/next
        logy_uc = self._logy_u(la_c, ls_c, gamma)  # [B,C,1,1]
        ...
        # L values & deltas
        val_uc = self._L_from_logy(logy_uc, tau_x)  # [B,C,1,1]
        ...
        if order == 1:
            ...
            if corrector:
                ...
                if i > 0:
                    ...
                    r_u = (val_uc - val_up) / (delta_u + self.eps)
                    r_v = (val_vc - val_vp) / (delta_v + self.eps)
                else:
                    r_u = r_v = torch.as_tensor(1.0, device=sample.device, dtype=sample.dtype)
                X += 0.5 * (xc - xp) * core_u / r_u.clamp_min(self.eps)
                E += 0.5 * (ec - ep) * core_v / r_v.clamp_min(self.eps)
    ...
    def sample(self, x, model_fn, **kwargs):
        ...
        xc, ec = self.checkpoint_model_fn(x_pred, timesteps[0])  # [B,C,H,W]
        B, C = xc.shape[:2]
        assert C == self.C, f"Channel mismatch: got {C}, expected {self.C}"

        # hidden init: head dtype/디바이스에 맞춤 + 배치 확장
        head_dtype = self.out.weight.dtype
        hidden = self.hidden_init.to(device=xc.device, dtype=head_dtype).expand(B, -1).contiguous()

        params, hidden = self.get_params(xc, hidden, timesteps[0:2])   # [B,2,5,C]
        ...
        for i in tqdm(range(self.steps), disable=not use_tqdm):
            ...
            # Predictor: params[:, 0, :, C]
            gamma = params[:, 0, 0]  # [B,C]
            tau_x = params[:, 0, 1]
            tau_e = params[:, 0, 2]
            kx    = params[:, 0, 3]
            ke    = params[:, 0, 4]
            ...
            # 안정화/브로드캐스트
            tau_x = torch.sigmoid(tau_x).unsqueeze(-1).unsqueeze(-1)  # [B,C,1,1]
            tau_e = torch.sigmoid(tau_e).unsqueeze(-1).unsqueeze(-1)
            gamma = gamma.unsqueeze(-1).unsqueeze(-1)
            kx    = kx.unsqueeze(-1).unsqueeze(-1)
            ke    = ke.unsqueeze(-1).unsqueeze(-1)

            x_pred = self.get_next_sample(
                x_corr, (xn, xc, xp), (en, ec, ep), i,
                log_alpha, log_sigma,
                log_alpha_ratio, log_sigma_ratio, log_alpha_next, log_sigma_next,
                gamma, tau_x, tau_e, kappa_x=kx, kappa_e=ke,
                order=p_order, eps=self.eps, corrector=False
            )

            if i < self.steps - 1:
                xn, en = self.checkpoint_model_fn(x_pred, timesteps[i + 1])
                params, hidden = self.get_params(xn, hidden, timesteps[i+1:i+3])  # [B,2,5,C]
            else:
                break
            ...
            if self.use_corrector:
                ...
                tau_x = torch.sigmoid(tau_x).unsqueeze(-1).unsqueeze(-1)
                tau_e = torch.sigmoid(tau_e).unsqueeze(-1).unsqueeze(-1)
                gamma = gamma.unsqueeze(-1).unsqueeze(-1)
                kx    = kx.unsqueeze(-1).unsqueeze(-1)
                ke    = ke.unsqueeze(-1).unsqueeze(-1)
                ...
                x_corr = self.get_next_sample(
                    ...
                    log_alpha_ratio, log_sigma_ratio, log_alpha_next, log_sigma_next,
                    gamma, tau_x, tau_e, kappa_x=kx, kappa_e=ke,
                    order=2, eps=self.eps, corrector=True
                )
            else:
                x_corr = x_pred

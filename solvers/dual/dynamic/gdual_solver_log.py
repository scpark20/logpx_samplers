...
class GDual_Solver(Solver):
    ...
    def get_next_sample(
        ...
        xn, xc, xp = xs
        en, ec, ep = es
        gamma, tau_x, tau_e, kappa_x, kappa_e = self.transform.unpack(params, self.eps)
        ...
        if order == 1:
            ...
            if corrector:
                ...
                r_u = (val_uc - val_up) / delta_u
                r_v = (val_vc - val_vp) / delta_v
                # self._nanlog(i, "r_u", r_u)
                # self._nanlog(i, "r_v", r_v)
                
                X = X + 0.5*(xc - xp)/r_u * first_and_second_u
                E = E + 0.5*(ec - ep)/r_v * first_and_second_v
    ...
    # ---------- sampling ----------
    def forward(self, x, model_fn, **kwargs):
...
# ---------- sampling ----------
    def sample(self, x, model_fn, **kwargs):
        ...
        with torch.no_grad():
            ...
            # 첫 모델 평가 + 파라미터
            xc, ec = self.model_fn(x_pred, timesteps[0])
            params, hidden = self.param_extractor({'x':xc, 'e':ec, 't': timesteps[0:2], 'h': None})
            ...
            for i in tqdm(range(self.steps), disable=not use_tqdm):
                ...
                if i < self.steps - 1:
                    xn, en = self.model_fn(x_pred, timesteps[i + 1])
                    params, hidden = self.param_extractor({'x':xn, 'e':en, 't': timesteps[i+1:i+3], 'h': hidden})
                else:
                    break

...
class GDualSolver(Solver):
    ...
    def __init__(
        ...
        self.feat_to_params = nn.Linear(latent_shape[0], 10)
        with torch.no_grad():
            self.feat_to_params.weight.zero_()
            self.feat_to_params.bias.zero_()
    ...
    # ---------- params ----------
    def get_params(self, xc):
        means = torch.mean(xc, dim=[2, 3])
        params = self.feat_to_params(means)
        params = params.reshape(len(xc), 2, 5)
        return params

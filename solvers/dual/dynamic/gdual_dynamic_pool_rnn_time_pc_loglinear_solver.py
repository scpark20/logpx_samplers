...
class GDual_Dynamic_Pool_RNN_Time_PC_LogLinear_Solver(Solver):
    """
    ...
    def __init__(
        ...
        self.feat = nn.Sequential(OrderedDict([
            ("c1x1", nn.Conv2d(C, 1, kernel_size=1, bias=True)),  # [B,1,H,W]
            ("flat", nn.Flatten(1)),                                # [B, H*W]
            ("proj", nn.Linear(H * W, hidden_dim)),                 # [B, hidden]
        ]))

...
class GDual_Solver(Solver):
    """
    GDual PC with Linear-Log  L(y, τ) = (1-τ)·log y + τ·y
    - u/v는 로그-도메인 계산으로 pow 제거
    - L, Δ 등은 로그/exp 조합으로 계산 (LambertW 불필요: L^{-1}(L(y))=y)
    ...
    def __init__(
        ...
        timesteps = self.get_time_steps(skip_type=skip_type, t_T=t_T, t_0=t_0,
                                        N=steps, device='cpu', shift=flow_shift)
    ...
    def _L_from_logy(logy: torch.Tensor, tau: torch.Tensor) -> torch.Tensor:
        # Linear-Log: L(y,τ) = (1-τ) log y + τ y, y = exp(logy)
    ...
    def get_params(self, xc, hidden, t_pair):
        ...
        return: [B, 2, 5]  # (predictor, corrector) x (γ, τx, τe, κx, κe)
        """
        B = xc.size(0)
    ...
    def get_next_sample(
        ...
        if order == 1:
            X = xc * ((y_un - y_uc) + self._O_delta_square(delta_u, kappa_x))
            E = ec * ((y_vn - y_vc) + self._O_delta_square(delta_v, kappa_e))
        else:
            # base (exact Δy terms)
            X = xc * (y_un - y_uc)
            E = ec * (y_vn - y_vc)

            # predictor-corrector(2차): ∫ g(u)(u-u_i) du ≈ Δu^2 / (2 * (τ + (1-τ)/y_i)) + ...
            core_u = (delta_u / (tau_x + (1.0 - tau_x) / y_uc)) + self._O_delta_square(delta_u, kappa_x)
            core_v = (delta_v / (tau_e + (1.0 - tau_e) / y_vc)) + self._O_delta_square(delta_v, kappa_e)

            if corrector:
                # x'θ(u_i) ≈ (xn - xc) / Δu_i  -> 0.5*(xn-xc)*core_u
                X += 0.5 * (xn - xc) * core_u
                E += 0.5 * (en - ec) * core_v
            else:
                # AB-like: x'θ(u_i) ≈ (xc - xp) / (u_i - u_{i-1})
                if i > 0:
                    ...
                    r_u = (val_uc - val_up) / (delta_u + eps)  # (u_i - u_{i-1}) / (u_{i+1} - u_i)

...
class GDual_PC_Hidden_Box_Solver(Solver):
    def __init__(
        self,
        model_fn,
        noise_schedule,
        ...
        if len(param_dim) > 0:
            ...
            self.hiddens = nn.Parameter(torch.zeros(steps, 2, 5, hidden_dim))
            self.hidden_to_param = nn.Parameter(torch.randn(hidden_dim, np.prod(param_dim)))
            self.param_shape = (steps, 2, 5, *param_dim)
        else:
            self.params = nn.Parameter(torch.zeros(steps, 2, 5))

    @property
    def params(self):
        return torch.reshape(self.hiddens @ self.hidden_to_param, self.param_shape) / np.sqrt(np.prod(self.param_dim))

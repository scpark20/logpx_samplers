...
class GDual_Time_PC_LogLinear_Plus2_Solver(Solver):
    def __init__(
        self,
        noise_schedule,
        steps,
    ...
    def get_next_sample(self, sample, xs, es, i, alphas, sigmas, gamma, tau_x, tau_e, kappa_x, kappa_e, order, eps=1e-8, corrector=False):
        ...
        if order == 1:
            X = xc * (L_inv_uc / ((1-tau_x)*L_inv_uc + tau_x) * delta_u + self.O_delta_square(delta_u, kappa_x))
            E = ec * (L_inv_vc / ((1-tau_e)*L_inv_vc + tau_e) * delta_v + self.O_delta_square(delta_v, kappa_e))
        
        else:
            X = xc * (L_inv_un - L_inv_uc)
            E = ec * (L_inv_vn - L_inv_vc)
            if corrector:
                X += 0.5 * (xn - xc) * (delta_u/((1-tau_x)+tau_x/L_inv_uc) + self.O_delta_square(delta_u, kappa_x))
                E += 0.5 * (en - ec) * (delta_v/((1-tau_e)+tau_e/L_inv_vc) + self.O_delta_square(delta_v, kappa_e))
            else:
                X += 0.5 * (xc - xp) * (delta_u/((1-tau_x)+tau_x/L_inv_uc) + self.O_delta_square(delta_u, kappa_x)) / r_u.clamp_min(eps)
                E += 0.5 * (ec - ep) * (delta_v/((1-tau_e)+tau_e/L_inv_vc) + self.O_delta_square(delta_v, kappa_e)) / r_v.clamp_min(eps) 

...
# ===== DCT helpers (2D, orthonormal) =====
def dct2(x: torch.Tensor) -> torch.Tensor:
    # DCT-II along last dim, then height dim
    x = torch.fft.dct(x, type=2, dim=-1, norm='ortho')
    x = torch.fft.dct(x, type=2, dim=-2, norm='ortho')
    return x

def idct2(X: torch.Tensor) -> torch.Tensor:
    # Inverse pair for DCT-II is also type=2 with norm='ortho'
    X = torch.fft.idct(X, type=2, dim=-1, norm='ortho')
    X = torch.fft.idct(X, type=2, dim=-2, norm='ortho')
    return X


class GDual_Time_PC_LogLinear_Plus2_DCT_Solver(Solver):
    """
    DCT-band (low/high) split sampler with learnable cutoff/transition per step/phase.
    - Nonlinear model calls remain in spatial domain.
    - Linear carryover/residual updates are combined in DCT domain per band.
    """
    ...
    def __init__(
        ...
        # ---- DCT band (learnable cutoff/transition) ----
        ...
        # DCT frequency grid uses f_x = u/(2W), f_y = v/(2H); diagonal r_max ~ sqrt(0.5^2+0.5^2)
        ...
        # Cache for DCT frequency radius grid
        self._freq_grid = {}  # key=(device, dtype, H, W) -> r-grid (H, W)
    ...
    # ===== DCT frequency radius grid r(u,v) ≈ sqrt((u/2W)^2 + (v/2H)^2) =====
    def _get_r_grid(self, H, W, device, dtype):
        key = (device, dtype, H, W)
        if key in self._freq_grid:
            return self._freq_grid[key]
        fy = (torch.arange(H, device=device, dtype=dtype) / (2.0 * H)).view(H, 1)  # [0, ~0.5)
        fx = (torch.arange(W, device=device, dtype=dtype) / (2.0 * W)).view(1, W)  # [0, ~0.5)
        r = torch.sqrt(fx**2 + fy**2)  # (H, W)
        self._freq_grid[key] = r
        return r

    # ===== Learnable low mask per (step i, phase) in DCT domain =====
    def _get_low_mask(self, i: int, phase: int, H: int, W: int, device, dtype):
        r = self._get_r_grid(H, W, device, dtype)  # (H, W)
        ...
        low = torch.sigmoid((c - r) / w)               # (H, W)
        return low[None, None, ...]                    # (1,1,H,W)

    # ===== DCT-domain linear combine (single pass) =====
    def _combine_bands_dct(self, sample, upd_low, upd_high, sc_low, sc_high, i: int, phase: int):
        """
        sample:   (B,C,H,W)
        ...
        M = self._get_low_mask(i, phase, H, W, sample.device, sample.dtype)     # (1,1,H,W)

        Xs = dct2(sample)                                                       # (B,C,H,W), real
        Hs = M * sc_low + (1.0 - M) * sc_high                                   # broadcast to (B,C,H,W)

        L  = dct2(upd_low)  * M
        Hh = dct2(upd_high) * (1.0 - M)
    ...
    def sample(self, x, model_fn, **kwargs):
        ...
        for i in tqdm(range(self.steps), disable=os.getenv("TQDM", "False")):
            ...
            x_pred = self._combine_bands_dct(
            ...
            x_corr = self._combine_bands_dct(
                sample=x_corr, upd_low=updL, upd_high=updH,

...
class GDual_Hybrid_Solver(Solver):
    def __init__(
        self,
        model_fn,
        noise_schedule,
    ...
    def u(self, alpha, sigma, gamma):
        if gamma >= 0:
            return torch.log(alpha * sigma**(-gamma)), True
        else:
            return alpha**(1 + gamma), False

    def v(self, alpha, sigma, gamma):
        if gamma >= 0:
            return sigma**(1 - gamma), False
        else:
            return torch.log(sigma * alpha**gamma), True

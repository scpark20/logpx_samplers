...
class Transform:
    ...
    def y(self, alpha, sigma, p, side):
        # alpha: (L,), sigma: (L,), gamma: (B, C)
        alpha = alpha[None, None, :]
        sigma = sigma[None, None, :]
        gamma = p['gamma'][:, :, None]

        if side == 'x':
            return torch.where(gamma >= 0, alpha * (sigma ** (-gamma)), alpha ** (1.0 + gamma))  # (B, C, L)
        else:
            return torch.where(gamma >= 0, sigma ** (1.0 - gamma), sigma * (alpha ** gamma))     # (B, C, L)
    ...
    def get_sample_grad_coeff(self, i, alphas, sigmas, alphas_ratio, sigmas_ratio, p):
        ...
        gamma = p['gamma']
        # sample_coeff : (B, C), grad_coeff : (B, C)

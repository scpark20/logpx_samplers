...
def main():
    ...
    if config.inception:
        # ⚠️ TF는 CPU 고정 (5090에서 INVALID_PTX 회피)
        tf_cfg = tf.ConfigProto(
            allow_soft_placement=True,
            device_count={'GPU': 0},                  # ← GPU 비활성화
            intra_op_parallelism_threads=int(os.getenv('TF_INTRA_OP', '2')),
            inter_op_parallelism_threads=int(os.getenv('TF_INTER_OP', '2')),
        )
        # rank0에서만 모델 파일 다운로드/준비
        if rank == 0:
            _sess0 = tf.Session(config=tf_cfg)
            _ = InceptionPool3(session=_sess0, batch_size=1)
            _sess0.close()
        barrier(local)

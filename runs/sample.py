...
def get_model(config: EasyDict):
    ...
    if config.model == 'GMDiT':
        ...
        return GMDiT()
    if config.model in ['FlowDCN-B-2', 'FlowDCN-XL-2']:
        from backbones.flowdcn import FlowDCN
        # model_id는 로컬 체크포인트 경로나 HF repo id로 교체
        return FlowDCN(model_id="path-or-repo-of-flowdcn")
